...
# Audit DFM8 transformation-expansion candidates with one Gemma 4 31B vLLM
# OpenAI-compatible judge server per GPU/family.
#
# This script audits data/dfm8_transform_expansion/<family>/data using the
# corresponding export/<family>/recreate_dataset.py audit implementation.
...
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/dfm8_transform_expansion_audits_$(date +%Y%m%dT%H%M%S)}"
...
for idx in "${!DATASETS[@]}"; do
  ...
  data_dir="$EXPANSION_ROOT/$dataset/data"
  audit_dir="$EXPANSION_ROOT/$dataset/$AUDIT_ROOT_NAME"

...
RUN_ID="${RUN_ID:-ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs131k-$(date +%Y%m%d-%H%M%S)}"
...
source /work/dfm/.home/miniforge3/etc/profile.d/conda.sh
conda activate hrm
RUN_ID="$RUN_ID" MASTER_PORT="$MASTER_PORT" scripts/resume_ferrum_xs_joint_10_more_epochs.sh
EOF

...
RUN_ID="${RUN_ID:-ferrum-strategy-xs-nondet-ctx16k-10epoch-val-$(date +%Y%m%d-%H%M%S)}"
LOG="logs/${RUN_ID}.log"
CKPT="checkpoints/ferrum_strategy_xs_nondet/${RUN_ID}"
MASTER_PORT="${MASTER_PORT:-29654}"

mkdir -p logs checkpoints/ferrum_strategy_xs_nondet
...
env WANDB_MODE=online OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
  ...
  pretrain.py \
  arch/size@arch=XS \
  data.path=data/sampled_ferrum_strategy_nondet_ctx16k_train \
  +data.validation_path=data/sampled_ferrum_strategy_nondet_ctx16k_validation \

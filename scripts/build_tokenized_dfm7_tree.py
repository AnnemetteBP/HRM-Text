#!/usr/bin/env python3
"""Build the selected DFM7 tokenized union tree."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-tokenized", type=Path, default=Path("data/tokenized_dfm7_jinja"))
    parser.add_argument("--base-tokenized", type=Path, default=Path("data/tokenized_dfm6"))
    parser.add_argument("--sapient-filtered", type=Path, default=Path("data/filtered_sources/sapient_cleaned"))
    parser.add_argument("--output", type=Path, default=Path("data/tokenized_dfm7"))
...
def selected_wrapper_name(raw_name: str, wrapper_prefix: str) -> str | None:
    if raw_name.startswith(wrapper_prefix):
        return raw_name.removeprefix(wrapper_prefix)
    return None
...
def main() -> None:
    ...
    if args.base_tokenized.exists():
        ...
        if base_info.read_text() != info.read_text():
            raise SystemExit(
                "Base tokenized tree and DFM7 additions use different tokenizer/template info. "
        ...
        for src in sorted(args.base_tokenized.iterdir()):
            if src.name == "tokenizer_info.json" or not src.is_dir():
                continue
            if src.name.startswith(EXCLUDED_PREFIXES):
                continue
    ...
    for src in task_dirs(args.raw_tokenized):
        ...
        if raw_name.startswith(EXCLUDED_PREFIXES):
            skipped += 1
            continue
        ...
        for wrapper_prefix in (DFM4_SUMMARIZATION_WRAPPER_PREFIX, DFM7_SPECIAL_WRAPPER_PREFIX):
            wrapped_name = selected_wrapper_name(raw_name, wrapper_prefix)
            if wrapped_name is not None:
                link_task(src, args.output, wrapped_name)
                selected += 1
                break
        else:

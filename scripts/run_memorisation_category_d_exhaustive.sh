...
SAMPLE="$REPO/logs/analysis/dfm9_memorisation_categories_cd_step1650000/prepared.jsonl.gz"
OUTPUT="$REPO/logs/analysis/dfm9_memorisation_category_d_exhaustive_step1650000"
...
if [[ ! -s "$OUTPUT/prepared_exhaustive_remaining.jsonl.gz" ]]; then
  echo "Preparing exhaustive Category D remainder..."
  "$PYTHON" "$SCRIPT" prepare-exhaustive \
    --categories D \
    --exclude-prepared "$SAMPLE" \
    --output-dir "$OUTPUT" \

...
RUN_ID="${RUN_ID:-ferrum-strategy-b-ctx16k-10epoch-val-$(date +%Y%m%d-%H%M%S)}"
LOG="logs/${RUN_ID}.log"
CKPT="checkpoints/ferrum_strategy_b/${RUN_ID}"
MASTER_PORT="${MASTER_PORT:-29646}"

mkdir -p logs checkpoints/ferrum_strategy_b

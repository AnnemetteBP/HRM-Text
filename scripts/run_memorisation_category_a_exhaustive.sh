...
OUTPUT="$REPO/logs/analysis/dfm9_memorisation_category_a_exhaustive_step1650000"
...
if [[ ! -s "$OUTPUT/prepared_exhaustive_remaining.jsonl.gz" ]]; then
  echo "Preparing exhaustive Category A remainder..."
...
for gpu in $(seq 0 7); do
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" "$SCRIPT" run-shard \
    ...
    --gpu-memory-utilization 0.90 --batch-size 256 \

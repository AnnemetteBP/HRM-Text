...
OUTPUT="$REPO/logs/analysis/dfm9_memorisation_category_c_exhaustive_step1650000"
...
if [[ ! -s "$OUTPUT/prepared_exhaustive_remaining.jsonl.gz" ]]; then
  echo "Missing prepared Category C input: $OUTPUT/prepared_exhaustive_remaining.jsonl.gz" >&2
  exit 1
fi

...
ckpt="checkpoints/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs131k-20260723-0830"
outroot="logs/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs131k-20260723-0830/eval_validation_epochs"
mkdir -p "$outroot"
...
for epoch in $(seq 11 20); do
  ...
  torchrun --nproc_per_node=8 --master_port=$((29700 + epoch)) \
...
out = Path("logs/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs131k-20260723-0830/eval_validation_epochs")
rows = []

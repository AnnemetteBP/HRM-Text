...
ckpt="checkpoints/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs262k-20260723-0858"
outroot="logs/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs262k-20260723-0858/eval_validation_epochs"
mkdir -p "$outroot"
...
for epoch in $(seq 11 20); do
  echo "eval-start epoch_${epoch} $(date -Is)" | tee -a "$outroot/run.stdout.txt"
  torchrun --nproc_per_node=8 --master_port=$((29750 + epoch)) \
...
out = Path("logs/ferrum_status_map_xs_joint/ferrum-status-map-joint-xs-ctx16k-20epoch-resume-gbs262k-20260723-0858/eval_validation_epochs")
rows = []
